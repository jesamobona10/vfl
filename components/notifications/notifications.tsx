  ...
  return (
    <div className="relative">
      ...
      >
        <Bell size={18} />
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 bg-danger text-white text-xs rounded-full px-1">
            {unreadCount}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="fixed left-3 right-3 top-16 z-50 sm:absolute sm:left-auto sm:right-0 sm:top-auto sm:mt-2 sm:w-80 floating-panel rounded-xl">
          <div className="p-3">
                ...
                <li key={n.id} className={`p-2 rounded ${n.is_read ? "" : "bg-accent/10"}`}>
                  <div className="text-sm">
                    {n.type === "player_transfer_in" && (
                      <span>
                        Player <strong>{n.payload?.player_name}</strong> joined your team.
                      </span>
                    )}
                    {n.type === "player_transfer_out" && (
                      <span>
                        Player <strong>{n.payload?.player_name}</strong> left your team.
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-muted mt-1 flex items-center justify-between">
                    ...
                    {!n.is_read && (
                      <button className="text-xs text-brand" onClick={() => markRead([n.id])}>
                        Mark read
                      </button>
                    )}
                  </div>
