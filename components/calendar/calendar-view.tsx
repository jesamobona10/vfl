  ...
  const matchesByDate = useMemo(() => {
    ...
    for (const round of fixtures) {
      for (const match of round.matches) {
        ...
        if (orgTeamIds && !orgTeamIds.includes(match.homeId) && !orgTeamIds.includes(match.awayId))
          continue;
      ...
      <div className="grid grid-cols-7 gap-px bg-line rounded-lg overflow-hidden">
        ...
        {Array.from({ length: totalCells }).map((_, i) => {
          ...
          return (
            ...
            >
              <span className={`${isToday ? "font-bold text-brand" : "text-text"}`}>{day}</span>
              {hasMatch && (
            ...
            selectedMatches.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  {m.time && <span className="text-muted shrink-0 ml-2">{m.time}</span>}
                </div>
              );
            })
          )}
