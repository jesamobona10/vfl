...
import { calculateStandings, allMatches, roundByeId } from "@/lib/logic/standings";
...
export function ReportView() {
  ...
  const report = useMemo(() => {
    ...
    fixtures.forEach((round) => {
      ...
      lines.push(`Round ${round.round} | Bye: ${teamName(byeId || 0)}`);
      round.matches.forEach((match) => {
        const score =
          match.status === "completed" && Number.isInteger(match.homeScore)
    ...
    calculateStandings(teams, fixtures).forEach((team, index) => {
      lines.push(
        `${String(index + 1).padStart(2, " ")}. ${team.name} - ${
          team.points
        } pts, GD ${team.gd}, P ${team.played}, Rating: ${team.rating?.toFixed(1) || "6.0"}`
      );
    });
