...
export function FixtureList({ loading = false }: { loading?: boolean }) {
  ...
  const handleDrop = (matchId: number, targetId: number) => {
    const round = fixtures.find((r) => r.matches.some((m) => m.id === matchId));
    if (round) {
    ...
    <div>
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted">Calendar and Team Views</p>
        </div>
      </div>
      ...
      {fixturesExist && (
        <div className="grid gap-3 grid-cols-2 lg:grid-cols-4 mb-6">
          <div className="card p-4 border border-line bg-surface">
            <p className="text-xs uppercase tracking-[0.2em] text-muted">Rounds</p>
          ...
          <div className="card p-4 border border-line bg-surface">
            <p className="text-xs uppercase tracking-[0.2em] text-muted">Live / In progress</p>
