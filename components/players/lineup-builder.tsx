...
export function LineupBuilder() {
  ...
  function setSlotPlayer(slotId: string, playerId: number | null) {
    setSlots((current) =>
      current.map((slot) => (slot.slotId === slotId ? { ...slot, playerId } : slot))
    );
  }
  ...
  async function handleDelete() {
    ...
    try {
      const response = await fetch(`/api/teams/${selectedTeamId}/lineups/${selectedLineupId}`, {
        method: "DELETE",
      });
      const data = await response.json();
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="card p-5 space-y-3">
          ...
          <div className="space-y-2 pt-3 text-sm">
            ...
            {isTeamAccount && !managedTeamId ? (
              <p className="text-sm text-accent">No managed team found for this account.</p>
            ) : null}
            {selectedTeam && (
        ...
        <div className="card p-5 space-y-3">
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl bg-surface-2 p-4 text-sm text-muted">
              ...
              <p>
                Assign players by their primary position and save the lineup to keep it across
                refresh.
              </p>
            </div>
          </div>
              ...
              {slots.map((slot) => {
                ...
                return (
                  <li key={slot.slotId} className="rounded-md bg-surface px-3 py-2 shadow-sm">
                    ...
                    {player ? `${player.name} #${player.number}` : "Unassigned"}
                  </li>
                );
              })}
            </ul>
