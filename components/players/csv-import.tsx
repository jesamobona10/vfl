    ...
    reader.onload = (ev) => {
      try {
        ...
        if (result.errors.length > 0) {
          msg += "<br>" + result.errors.map((er) => `&bull; ${er}`).join("<br>");
        }
        setFeedback({
          type: result.errors.length === 0 && result.imported.length > 0 ? "success" : "error",
          message: msg,
        });
      } catch (err: any) {
        setFeedback({
      ...
      {feedback && (
        <div
          className={`fixed bottom-4 right-4 max-w-sm p-4 rounded-lg shadow-lg z-50 ${
            feedback.type === "success" ? "bg-brand text-white" : "bg-danger text-white"
          }`}
        >
          <div className="flex items-start gap-2">
            ...
            <button onClick={() => setFeedback(null)} className="shrink-0 ml-2">
              <X size={16} />
            </button>
          </div>
        </div>
