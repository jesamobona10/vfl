...
export function PlayerCard({ player, teamName, onEdit, onDelete }: PlayerCardProps) {
  ...
  const CompactStats = () => {
    ...
    if (pos === "GK") {
      ...
      items.push({ label: "G", value: player.goals }, { label: "A", value: player.assists });
    }
    ...
    return (
      <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-muted">
        ...
        <span className="font-semibold text-text">{player.rating.toFixed(1)}</span>
      </div>
    );
  };
        ...
        <div className="p-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              ...
              {player.captain && <Crown size={12} className="text-accent shrink-0" />}
            </div>
        ...
        <div className="border-t border-line px-4 pb-4 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            ...
            {stat(
              "Yellow Cards",
              player.yellowCards,
              <div className="w-2.5 h-3.5 rounded-sm bg-gold-500" />
            )}
            {stat(
              "Red Cards",
              player.redCards,
              <div className="w-2.5 h-3.5 rounded-sm bg-danger-500" />
            )}
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                ...
                {stat(
                  "Clean Sheets",
                  player.cleanSheets,
                  <Trophy size={13} className="text-live-500" />
                )}
                {stat(
                  "Goals Conceded",
                  player.goalsConceded,
                  <Goal size={13} className="text-danger-500" />
                )}
                {stat(
                  "Penalty Saves",
                  player.penaltySaves,
