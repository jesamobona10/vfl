...
const formationCounts: Record<
  FormationType,
  { GK: number; DEF: number; MID: number; ATT: number }
> = {
...
function getPlayerRoundScore(playerId: number, matchIds: number[]): number {
...
export function TeamOfRound() {
  ...
  const handleDownload = () => {
    ...
    downloadFormationImage(bestGK || null, bestDEF, bestMID, bestATT, roundNum, formation);
  };
  ...
  return (
    <div>
      <div className="flex flex-col gap-3 md:flex-row md:items-center mb-4">
        ...
        {roundNum && (
          <button onClick={handleDownload} className="btn-ghost" type="button">
            <Download size={16} />
            Download
          </button>
        )}
...
function FormationRow({ label, players, count }: { label: string; players: any[]; count: number }) {
  return (
