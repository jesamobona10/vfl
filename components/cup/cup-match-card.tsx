    ...
    >
      <button
        ...
        title={
          isCompleted
            ? "View result"
            : isTbd(homeTeamId) || isTbd(awayTeamId)
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-sm font-medium truncate ${isTbd(homeTeamId) ? "text-muted italic" : "text-text"}`}
              >
                {homeName}
              </span>
            </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            {displayScore ? (
              <>
                <span
                  className={`text-base font-bold tabular-nums ${isCompleted && match.winnerId === homeTeamId ? "text-brand" : "text-text"}`}
                >
                  {match.homeScore}
                </span>
                <span className="text-muted text-xs">-</span>
                <span
                  className={`text-base font-bold tabular-nums ${isCompleted && match.winnerId === awayTeamId ? "text-brand" : "text-text"}`}
                >
                  {match.awayScore}
                </span>
              </>
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 justify-end">
              <span
                className={`text-sm font-medium truncate ${isTbd(awayTeamId) ? "text-muted italic" : "text-text"}`}
              >
                {awayName}
              </span>
              {awayLogo ? (
