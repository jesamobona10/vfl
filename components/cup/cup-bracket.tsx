  ...
  return (
    <div className="card p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-5">
        <h3 className="text-sm font-semibold text-muted uppercase tracking-wider">Cup Bracket</h3>
        {champion != null && (
      ...
      <div className="md:hidden space-y-6">
        {roundConfig.map((round, ri) => {
          const columnMatches = bracketMatches.filter((m) => m.round === round.key);
          ...
          return (
            <div key={round.key}>
              ...
              <div className="space-y-2">
                {columnMatches.map((m) => {
                  const isPlayable = m.homeId != null && m.awayId != null;
            ...
            <div key={round.key} className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                {ri < roundConfig.length - 1 && (
                  <ChevronRight size={14} className="text-muted/30 hidden md:block" />
                )}
              </div>
