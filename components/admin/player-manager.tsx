  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const body = {
        ...
        jersey_number: Number(formNumber),
      };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => useAppStore.getState().recalculateRatings()}
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card p-4 space-y-3 border-l-4 border-l-brand">
          ...
          <div className="flex gap-2">
            <button
              type="submit"
              disabled={submitting || !formName || !formTeamId}
              className="btn-primary text-sm"
            >
              {submitting ? (
        ...
        <div className="space-y-1">
          {players.map((p) => (
            <div key={p.id} className="card px-4 py-2 flex items-center gap-3 text-sm">
              ...
              <button onClick={() => handleDelete(p)} className="btn-ghost text-xs text-danger">
                <Trash2 size={12} />
              </button>
            </div>
          ))}
