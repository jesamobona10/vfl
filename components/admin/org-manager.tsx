      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card p-4 space-y-3 border-l-4 border-l-brand">
          <h4 className="text-sm font-semibold text-muted uppercase">
            {editingOrg ? "Edit" : "Create"} Organization
          </h4>
            ...
            <div className="flex items-center gap-3 pt-1">
              ...
              >
                {editingLogoUrl || editingOrg.logo_url ? (
                  <img
                    src={editingLogoUrl || editingOrg.logo_url!}
                    alt="Logo"
              ...
              <div className="card px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted">
                    ...
                    {org.teamCount} teams &middot; {org.memberCount} members &middot;{" "}
                    {org.competitionCount} comps
                  </p>
                </div>
                ...
                <div className="ml-6 mt-1 mb-2 card p-3 space-y-3">
                  <div className="flex items-center justify-between">
                    <h5 className="text-xs font-semibold text-muted uppercase">
                      Members ({members[org.id]?.length || 0})
                    </h5>
                    <button
                      onClick={() => setMemberFormOrgId(org.id)}
                      className="btn-ghost text-xs"
                    >
                      <Plus size={12} /> Add
                    </button>
                  ...
                  {memberFormOrgId === org.id && (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => setMemberFormOrgId(null)}
                        className="btn-ghost text-xs"
                      >
                        <X size={12} />
                      </button>
                  ...
                  {membersLoading === org.id ? (
                    ...
                    <div className="space-y-1">
                      {members[org.id].map((m) => (
                        <div
                          key={m.id}
                          className="flex items-center gap-2 text-xs py-1.5 px-2 rounded-lg bg-surface-2"
                        >
                          <span className="font-mono flex-1 truncate">{m.user_id}</span>
                          <select
                            value={m.role}
                            onChange={(e) => handleChangeRole(org.id, m.user_id, e.target.value)}
