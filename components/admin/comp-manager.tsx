...
export function CompManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingComp
        ? `/api/admin/competitions/${editingComp.id}`
  ...
  const handleActivateSeason = async (season: SeasonRow) => {
    ...
    try {
      ...
      if (!res.ok) {
        const d = await res.json();
        setError(d.error || "Failed to activate season.");
        return;
      }
      await fetchSeasons(editingComp.id);
    } catch {
  ...
  const handleCompleteSeason = async (season: SeasonRow) => {
    ...
    try {
      ...
      if (!res.ok) {
        const d = await res.json();
        setError(d.error || "Failed to complete season.");
        return;
      }
      await fetchSeasons(editingComp.id);
    } catch {
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card p-4 space-y-3 border-l-4 border-l-brand">
            ...
            <div className="flex items-center gap-3 pt-1">
              ...
              >
                {editingLogoUrl || editingComp.logo_url ? (
                  <img
                    src={editingLogoUrl || editingComp.logo_url!}
                    alt="Logo"
              ...
              {seasonsOpen && (
                <div className="space-y-2">
                  ...
                  {seasons.map((s) => (
                    <div
                      key={s.id}
                      className="flex items-center justify-between p-2 rounded bg-surface-2 text-xs"
                    >
          ...
          <div className="flex gap-2">
            <button
              type="submit"
              disabled={submitting || !formName || (!editingComp && !formOrgId)}
              className="btn-primary text-sm"
            >
              {submitting ? (
        ...
        <div className="space-y-1">
          {comps.map((comp) => (
            <div key={comp.id} className="card px-4 py-2.5 flex items-center gap-3 text-sm">
              ...
              <button onClick={() => openEditWithSeasons(comp)} className="btn-ghost text-xs">
                <Edit2 size={12} />
              </button>
              <button onClick={() => handleDelete(comp)} className="btn-ghost text-xs text-danger">
                <Trash2 size={12} />
              </button>
            </div>
          ))}
