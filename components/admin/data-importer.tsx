...
export function DataImporter() {
  ...
  const fixtureCount = plan ? plan.fixtures.reduce((s, r) => s + r.matches.length, 0) : 0;
    ...
    return (
      <div className="space-y-6">
        <div className="text-center py-6 space-y-4">
          ...
          <p className="text-xs text-muted">{summary.mapped} teams were auto-matched by name.</p>
        </div>
        ...
        <div className="card p-4">
          ...
          {syncStatus === "idle" && <p className="text-xs text-muted">Syncing to database...</p>}
        </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted">
          Replace all existing data with data from a JSON export file. This cannot be undone.
        </p>
      </div>
      ...
      {step === "upload" && (
        <div className="space-y-4">
          ...
          >
            ...
            <p className="font-medium">Drop a JSON export file here or click to browse</p>
            <p className="text-sm text-muted mt-1">Accepted format: VFL export JSON (.json)</p>
          </div>
          ...
          />
          <button onClick={handlePaste} className="btn-primary" disabled={!jsonText.trim()}>
                  ...
                  <div key={t.id} className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm">
                    ...
                    {srcId !== undefined && srcId !== t.id && (
                      <>
                        ...
                        <span className="text-xs text-muted">source ID: {srcId}</span>
                      </>
                    )}
                    {wasMatched ? (
                      <span className="text-xs text-brand ml-auto">Auto-matched</span>
                    ) : (
                      <span className="text-xs text-muted ml-auto">New team</span>
                    )}
                  </div>
                );
              })}
      ...
      {step === "confirm" && plan && (
        <div className="space-y-6">
          <div className="bg-danger/10 border border-danger/20 rounded-xl p-6 text-center space-y-3">
            ...
            <h4 className="text-lg font-bold text-danger">This will replace ALL existing data</h4>
            <p className="text-sm text-muted">
              Current data ({teams.length} teams, {useAppStore.getState().players.length} players,{" "}
              {useAppStore.getState().fixtures.length} fixture rounds) will be permanently
              overwritten.
            </p>
          </div>
          ...
          <div className="flex gap-3 justify-center">
            ...
            <button onClick={handleImport} disabled={importing} className="btn-danger">
              {importing ? (
                <>
