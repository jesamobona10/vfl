...
export function generateCupBracket(
  ...
  const teamName = (id: number) => standings.find((s) => s.id === id)?.name || `Team ${id}`;
  ...
  const qfData: {
    ...
    {
      homeId: autoQualifiers[0].id,
      awayId: null,
      awayFromMatchId: playoffMatches[2]?.id,
      pairing: pairingByPlayoffIndex[2],
    },
    { homeId: autoQualifiers[3].id, awayId: autoQualifiers[4].id },
    {
      homeId: autoQualifiers[1].id,
      awayId: null,
      awayFromMatchId: playoffMatches[1]?.id,
      pairing: pairingByPlayoffIndex[1],
    },
    {
      homeId: autoQualifiers[2].id,
      awayId: null,
      awayFromMatchId: playoffMatches[0]?.id,
      pairing: pairingByPlayoffIndex[0],
    },
  ];
