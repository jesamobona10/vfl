...
export function validateFixtureStructure(fixtures: FixtureRound[], teams: Team[]): string[] {
  const errs: string[] = [];
  ...
  fixtures.forEach((r) => {
    ...
    r.matches.forEach((m) => {
      ...
      if (pairs.has(key)) errs.push(`Duplicate pairing in round ${r.round} match ${m.id}.`);
      pairs.add(key);
...
export function verifyFixtures(fixtures: FixtureRound[], teams: Team[]): VerifyResult {
  const errors: string[] = [];
