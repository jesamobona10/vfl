...
export function liveSettings(
  competitionSettings?: LiveClockSettings | Record<string, unknown>
): Required<LiveClockSettings> {
...
export function livePhase(
  ...
  if (elapsedMs < halftimeStart) {
    ...
    const stoppageSecond = inStoppage
      ? Math.min(Math.floor((elapsedMs - halfMs) / MIN_MS), settings.stoppageMinutes - 1)
      : 0;
    return {
      label: "1H",
  ...
  if (elapsedMs < fullTimeMs) {
    ...
    const stoppageSecond = inStoppage
      ? Math.min(Math.floor((playElapsed - halfMs) / MIN_MS), settings.stoppageMinutes - 1)
      : 0;
  ...
  return {
    label: "FT",
    minute: 90,
    stoppage: `+${settings.stoppageMinutes}`,
    seconds: 0,
    elapsedMs,
    done: true,
  };
...
export function matchKickoff(
  ...
  const utcMs =
    Date.UTC(year, month - 1, day, Math.floor(mins / 60), mins % 60) + tzOffsetMinutes * MIN_MS;
...
export function isLiveEligible(
  ...
  return (
    current >= kickoffMs - leadMinutes * MIN_MS && current <= kickoffMs + graceMinutes * MIN_MS
  );
}
