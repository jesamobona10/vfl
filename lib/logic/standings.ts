...
export function standingsMatches(fixtures: FixtureRound[]): Match[] {
  return allMatches(fixtures).filter(
    (match) =>
      (match.status === "completed" &&
        Number.isInteger(match.homeScore) &&
...
export function roundByeId(round: FixtureRound, teams: Team[]): number | null {
  const activeTeams = new Set(round.matches.flatMap((match) => [match.homeId, match.awayId]));
  const missingTeams = teams.filter((team) => !activeTeams.has(team.id));
  return missingTeams.length === 1 ? missingTeams[0].id : round.byeId;
}

export function calculateStandings(teams: Team[], fixtures: FixtureRound[]): StandingRow[] {
  ...
  standingsMatches(fixtures).forEach((match) => {
...
export function leagueStats(teams: Team[], fixtures: FixtureRound[]): LeagueStats {
  const completed = completedMatches(fixtures);
  const goals = completed.reduce((total, match) => total + match.homeScore! + match.awayScore!, 0);
  ...
  return {
    goals,
    goalsPerMatch: completed.length ? (goals / completed.length).toFixed(2) : "0.00",
    biggestWin:
