...
export function swapTeamsAcrossFixtures(
  ...
  allMatches(fixtures).forEach((candidate) => {
    ...
    if (candidate.homeId !== previousHomeId || candidate.awayId !== previousAwayId) {
...
export function repairFixturesFromLocks(fixtures: FixtureRound[], teams: Team[]): RepairResult {
  if (!fixtures.length) {
  ...
  const openMatches = matches.filter((match) => !lockedMatches.includes(match));
  const usedPairs = new Set<string>();
  const roundTeams = new Map(fixtures.map((round) => [round.round, new Set<number>()]));
  const originalPairs = new Map(matches.map((match) => [match.id, [match.homeId, match.awayId]]));
  ...
  const remainingPairKeys = new Set([...pairLookup.keys()].filter((key) => !usedPairs.has(key)));
  ...
  function candidatesFor(match: Match) {
    ...
    return [...remainingPairKeys]
      ...
      .filter(([homeId, awayId]) => !teamsInRound.has(homeId) && !teamsInRound.has(awayId))
      .sort((a, b) => {
  ...
  if (!solve()) {
    if (solveIterations > MAX_SOLVE_ITERATIONS || Date.now() - solveStartTime > SOLVE_TIMEOUT_MS) {
      return {
        ok: false,
        reason:
