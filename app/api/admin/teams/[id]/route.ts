...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const limited = rateLimit({ key: `admin:teams:update:${ip}:${auth!.userId}`, limit: 60, windowMs: 60 * 60_000 });
    if (limited.limited) {
      logSecurityEvent("admin_team_update_rate_limited", { ip, userId: auth!.userId });
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const ip = getClientIp(request);
    const limited = rateLimit({ key: `admin:teams:delete:${ip}:${auth!.userId}`, limit: 60, windowMs: 60 * 60_000 });
    if (limited.limited) {
      logSecurityEvent("admin_team_delete_rate_limited", { ip, userId: auth!.userId });
      return rateLimitResponse(limited.resetAt);
    }
