...
export async function POST(request: Request) {
  try {
    ...
    const limited = await rateLimit({
      key: `admin:teams:create:${ip}:${auth!.userId}`,
      limit: 60,
      windowMs: 60 * 60_000,
    });
    if (limited.limited) {
      logSecurityEvent("admin_team_create_rate_limited", { ip, userId: auth!.userId });
    ...
    if (!name || name.length > 80)
      return json({ error: "Team name is required (max 80 chars)." }, { status: 400 });
    ...
    const { data, error } = await sb
      .from("teams")
      .insert({ name, organization_id, rating })
      .select()
      .single();
