...
export async function POST(request: Request) {
  try {
    ...
    if (!name || name.length > 100)
      return json({ error: "Player name is required (max 100 chars)." }, { status: 400 });
    if (!team_id) return json({ error: "Team ID is required." }, { status: 400 });
    if (!["GK", "DEF", "MID", "ATT"].includes(position))
      return json({ error: "Position must be GK, DEF, MID, or ATT." }, { status: 400 });
    ...
    const { data, error } = await sb
      .from("players")
      .insert({ name, team_id, position, jersey_number })
      .select()
      .single();
