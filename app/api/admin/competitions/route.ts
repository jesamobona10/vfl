...
export async function POST(request: Request) {
  try {
    ...
    const limited = await rateLimit({
      key: `admin:competitions:create:${ip}:${auth!.userId}`,
      limit: 60,
      windowMs: 60 * 60_000,
    });
    if (limited.limited) {
      logSecurityEvent("admin_competition_create_rate_limited", { ip, userId: auth!.userId });
    ...
    if (!name || name.length > 100)
      return json({ error: "Competition name is required (max 100 chars)." }, { status: 400 });
    ...
    const { data, error } = await sb
      .from("competitions")
      .insert({ name, organization_id, type, season, status: "draft", created_by: auth!.userId })
      .select()
      .single();
