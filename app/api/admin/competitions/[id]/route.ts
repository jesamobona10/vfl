...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const limited = rateLimit({
      key: `admin:competitions:update:${ip}:${auth!.userId}`,
      limit: 60,
      windowMs: 60 * 60_000,
    });
    if (limited.limited) {
    ...
    if (parsed.data!.type !== undefined) {
      if (!["league", "cup", "friendly"].includes(parsed.data!.type as string))
        return json({ error: "Invalid type." }, { status: 400 });
      update.type = parsed.data!.type;
    }
    if (parsed.data!.status !== undefined) {
      if (!["draft", "active", "completed", "archived"].includes(parsed.data!.status as string))
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const ip = getClientIp(request);
    const limited = rateLimit({
      key: `admin:competitions:delete:${ip}:${auth!.userId}`,
      limit: 60,
      windowMs: 60 * 60_000,
    });
    if (limited.limited) {
