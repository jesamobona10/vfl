...
export async function GET(request: Request) {
  try {
    ...
    const { data: fixtures, error: fixturesError } = await sb
      .from("fixtures")
      .select(
        "*, home:home_team_id!inner(name, logo_url, organization_id), away:away_team_id!inner(name, logo_url, organization_id)"
      )
      .order("round")
      .order("id");
    ...
    const { data: dbEvents } = await sb.from("match_events").select("*").in("match_id", fixtureIds);
    ...
    const { data: orgs } = await sb.from("organizations").select("id, name, slug, logo_url");
