...
export async function DELETE(request: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const orgId =
      (account.teams as unknown as { organization_id: string } | null)?.organization_id || null;

    const { error: deleteError } = await sb.auth.admin.deleteUser(params.id);

    if (deleteError) {
      logApiError("admin_team_account_delete_failed", deleteError, {
        userId: auth!.userId,
        accountId: params.id,
      });
    ...
    writeAuditEvent("team_account_deactivated", auth!.userId, orgId || undefined, {
      ip,
      accountId: params.id,
      username: account.username,
    });
