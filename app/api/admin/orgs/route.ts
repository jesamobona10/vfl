...
export async function GET() {
  try {
    ...
    const orgsWithCounts = await Promise.all(
      (orgs || []).map(async (org) => {
        const [{ count: teamCount }, { count: memberCount }, { count: compCount }] =
          await Promise.all([
            sb
              .from("teams")
...
export async function POST(request: Request) {
  try {
    ...
    const limited = await rateLimit({
      key: `admin:orgs:create:${ip}:${auth!.userId}`,
      limit: 60,
      windowMs: 60 * 60_000,
    });
    if (limited.limited) {
    ...
    if (!name || name.length > 100)
      return json({ error: "Organization name is required (max 100 chars)." }, { status: 400 });
    if (!["school", "academy", "club"].includes(type))
      return json({ error: "Type must be school, academy, or club." }, { status: 400 });
    if (!slug || slug.length < 2 || slug.length > 60)
      return json(
        { error: "Slug is required (2-60 chars, lowercase alphanumeric with hyphens)." },
        { status: 400 }
      );
    ...
    const { data: existing } = await sb
      ...
      .maybeSingle();
    if (existing)
      return json({ error: "An organization with this slug already exists." }, { status: 409 });
