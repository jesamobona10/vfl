...
export async function GET() {
  try {
    ...
    const { data: dbTeams } = await sb.from("teams").select("*").order("id");
    ...
    const { data: dbPlayers } = await sb.from("players").select("*").order("id");
    ...
    const { data: dbMatches } = await sb.from("fixtures").select("*").order("round").order("id");
