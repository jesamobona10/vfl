...
export async function POST(request: Request) {
  try {
    ...
    if (!players || !Array.isArray(players)) {
      return json({ error: "Players array is required." }, { status: 400 });
    }

    if (players.length > MAX_SYNC_SIZE) {
      return json({ error: `Too many players. Maximum is ${MAX_SYNC_SIZE}.` }, { status: 400 });
    }
    ...
    const { data: synced } = await sb.from("players").select("*").order("id");
