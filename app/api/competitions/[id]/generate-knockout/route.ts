...
import { generatePlayoffs, generateCupBracket, resetCupIdCounter } from "@/lib/logic/cup";
...
export async function POST(
  ...
  try {
    ...
    if (!dbTeams || dbTeams.length < 11) {
      return json({ error: "Need at least 11 teams in the organization." }, { status: 400 });
    }
    ...
    const allCupMatches = [...playoffMatches, ...bracketMatches];
