...
export async function GET(
  ...
  try {
    ...
    const playerMap = new Map((players || []).map((p: any) => [p.id, { name: p.name, teamId: p.team_id }]));
    ...
    const goalMap = new Map<number, { playerId: number; name: string; teamId: number; teamName: string; count: number }>();
    const assistMap = new Map<number, { playerId: number; name: string; teamId: number; teamName: string; count: number }>();
    const yellowMap = new Map<number, { playerId: number; name: string; teamId: number; teamName: string; count: number }>();
    const redMap = new Map<number, { playerId: number; name: string; teamId: number; teamName: string; count: number }>();

    const increment = (map: Map<number, any>, playerId: number, name: string, teamId: number, teamName: string) => {
