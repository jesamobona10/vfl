...
export async function POST(request: Request) {
  try {
    ...
    if (!email || !isValidEmail(email) || passwordError) {
      ...
      return json({ error: passwordError || "A valid email is required." }, { status: 400 });
    }

    const sb = createServiceRoleClient();

    const { data: existingAdmins } = await sb.from("admin_users").select("id").limit(1);

    if (existingAdmins && existingAdmins.length > 0) {
      logSecurityEvent("blocked_extra_admin_signup", { ip, email });
      return json({ error: "Admin already exists. Use login instead." }, { status: 409 });
    }
