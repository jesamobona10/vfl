  ...
  try {
    ...
    if (authError) {
      ...
      void writeAuditRecord({
        ...
        actorRole: "anonymous",
        resourceType: "AUTH",
        description: `Failed org login for ${email}`,
        success: false,
        ip,
      }).catch(() => {});
    ...
    if (!memberships || memberships.length === 0) {
      ...
      return json({ error: "No organization found for this account." }, { status: 403 });
    }
    ...
    void writeAuditRecord({
      ...
      actorRole: `org_${memberships[0].role}`,
      organizationId: memberships[0].organization_id,
      resourceType: "AUTH",
      description: `Org login succeeded for ${email} (${org.slug})`,
      success: true,
      ip,
    }).catch(() => {});
