...
async function checkEventOwnership(
  ...
  const isTeamOwner =
    ownsTeam(authed, fixture.home_team_id) || ownsTeam(authed, fixture.away_team_id);
...
export async function DELETE(request: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (eventIdParam) {
      ...
      const { error } = await supabase.from("match_events").delete().eq("id", eventIdParam);
    ...
    const { error } = await supabase.from("match_events").delete().eq("id", existing[0].id);
