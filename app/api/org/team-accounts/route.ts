...
export async function GET(request: Request) {
  try {
    ...
    const { data: teams } = await sb.from("teams").select("id").eq("organization_id", orgId);
...
export async function POST(request: Request) {
  try {
    ...
    if (!teamNameInput || passwordError) {
      return json({ error: passwordError || "Team name is required." }, { status: 400 });
    }
    ...
    logSecurityEvent("org_team_account_created", {
      ip,
      userId: auth!.userId,
    ...
    void writeAuditRecord({
      ...
      description: `Created team account ${username} for ${teamName}`,
      after: { username, teamId, teamName },
      ip,
    }).catch(() => {});
    return json({
      success: true,
