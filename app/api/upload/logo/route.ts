...
export async function POST(request: Request) {
  try {
    ...
    if (!file || !teamId || !teamName) {
      return json({ error: "File, teamId, and teamName are required." }, { status: 400 });
    }
    ...
    const limited = await rateLimit({
      key: `upload:logo:${ip}:${auth!.userId}`,
      limit: 30,
      windowMs: 60 * 60_000,
    });
    if (limited.limited) {
    ...
    const { error: uploadError } = await sb.storage.from("team-logos").upload(fileName, buffer, {
    ...
    const { data: urlData } = sb.storage.from("team-logos").getPublicUrl(fileName);
