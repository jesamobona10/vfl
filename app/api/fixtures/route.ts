...
import type { FixtureRound, Match, MatchEvent, FixtureRow, MatchEventRow, Team } from "@/lib/types";
...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase.from("fixtures").select("*, match_events(*)").order("round").order("id");
    ...
    if (orgId) {
      ...
      if (teamIds.length > 0) {
        const conditions = teamIds.flatMap((id) => [
          `home_team_id.eq.${id}`,
          `away_team_id.eq.${id}`,
        ]);
    ...
    const rawFixtures: (FixtureRow & { match_events?: MatchEventRow[] })[] = data || [];

    const grouped = new Map<number, Match[]>();
    const roundSet = new Set<number>();

    for (const m of rawFixtures) {
      const events: MatchEvent[] = (m.match_events || []).map((e) => ({
...
export async function POST(request: Request) {
  try {
    ...
    if (!round || !homeTeamId || !awayTeamId || homeTeamId === awayTeamId) {
      return json(
        { error: "Valid round, home team, and away team are required." },
        { status: 400 }
      );
    }
