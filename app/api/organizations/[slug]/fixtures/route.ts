...
export async function GET(request: Request, { params }: { params: { slug: string } }) {
  try {
    ...
    let query = sb.from("fixtures").select("*").order("round").order("id");
    ...
    if (competitionId) {
      ...
      // Scope to all competition seasons that belong to this org season
      ...
      const compIds = (orgComps || []).map((c) => c.id);
      if (compIds.length > 0) {
        query = query.in("competition_id", compIds).in("season_id", seasonIds);
      } else {
        return json({ fixtures: [] });
      }
    } else {
