...
const tabs = [
  { href: "", label: "Overview", icon: LayoutGrid, showFor: null },
  { href: "teams", label: "Teams", icon: Shield, showFor: null },
  { href: "fixtures", label: "Fixtures", icon: Calendar, showFor: null },
  { href: "results", label: "Results", icon: ListOrdered, showFor: null },
  { href: "standings", label: "Standings", icon: Trophy, showFor: "league" },
  { href: "players", label: "Players", icon: Users, showFor: null },
  { href: "stats", label: "Statistics", icon: BarChart3, showFor: null },
  { href: "live", label: "Live Event", icon: Activity, showFor: null },
...
export default function CompetitionLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // propagate selected season to global store so other components can react
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex flex-col gap-2 min-w-0">
          ...
          <div className="flex items-center gap-3">
            ...
            {/** Display active/selected season name (avoid legacy competition.season) */}
            {(() => {
              const selected = seasons.find((s) => s.id === selectedSeasonId) || currentSeason;
              return selected ? <span className="text-sm text-muted">{selected.name}</span> : null;
            })()}
          </div>
        </div>
      </div>
